...
//        ApplicationListener<ApplicationContextInitializedEvent> { era assim quando era um Listener
//@Component
public class Base64ProtocolResolver implements ApplicationContextInitializer<ConfigurableApplicationContext> {
...
//    }
    ...
    public void initialize(ConfigurableApplicationContext applicationContext) {
        applicationContext.addProtocolResolver((String location, ResourceLoader resourceLoader) -> {
            if (location.startsWith("base64:")) {
            byte[] decodeResource = Base64.getDecoder().decode(location.substring(7));
            return new ByteArrayResource(decodeResource);
        }
